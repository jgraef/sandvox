...
impl Aabb {
    ...
    pub fn center(&self) -> Point3<f32> {
        (0.5 * (self.min.coords + self.max.coords)).into()
    }
    ...
    pub fn transform_with(&mut self, similarity: Similarity3<f32>) {
        self.min = similarity.transform_point(&self.min);
        self.max = similarity.transform_point(&self.max);
    }
}
...
impl FromIterator<Aabb> for Aabb {
    fn from_iter<T: IntoIterator<Item = Aabb>>(iter: T) -> Self {
        let mut iter = iter.into_iter();
        let mut output = iter.next().expect("Need atleast 1 AABB in interator");
        for aabb in iter {
            output.merge(aabb);
        }
        output
